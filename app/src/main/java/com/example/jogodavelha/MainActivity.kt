...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateGame(values: MutableList<Characters>) {
        binding.apply {
            ...
            playButtons.forEachIndexed { index, button ->
                setCorrectChar(button, values[index])
            }
        }
    }

    private fun setCorrectChar(house: MaterialButton, characters: Characters) {
        when (characters) {
            Characters.DOT -> house.updateTextWithAnimation(getString(R.string.dot))
            Characters.CROSS -> house.updateTextWithAnimation(getString(R.string.cross))
            Characters.EMPTY -> house.updateTextWithAnimation("")
        }
    }
